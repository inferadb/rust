...
use crate::common::{TestFixture, validate_environment};

/// Test that the dev environment is accessible
#[tokio::test]
async fn test_environment_health() {
    validate_environment().await.expect("Dev environment should be running and healthy");
}
...
async fn test_fixture_creation() {
    ...
    assert!(!fixture.cert_kid.is_empty(), "Certificate KID should not be empty");
...
async fn test_jwt_generation() {
    ...
    let jwt = fixture.generate_jwt(None, &[]).expect("JWT generation should succeed");
...
async fn test_sdk_client_creation() {
    ...
    assert!(client.is_ok(), "SDK client creation should succeed: {:?}", client.err());
