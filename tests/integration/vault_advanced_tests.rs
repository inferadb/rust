...
async fn test_simulate_check() {
    ...
    match result {
        Ok(simulation_result) => {
            ...
            assert!(
                simulation_result.allowed,
                "Should be allowed with simulated relationship"
            );
        }
        Err(e) => {
...
async fn test_simulate_multiple_relationships() {
    ...
    match result {
        Ok(simulation_result) => {
            println!(
                "Multi-relationship simulation result: {:?}",
                simulation_result
            );
        }
        Err(e) => {
...
async fn test_simulate_remove_relationship() {
    ...
    match result {
        Ok(simulation_result) => {
            ...
            assert!(
                !simulation_result.allowed,
                "Should be denied after simulated removal"
            );
        }
        Err(e) => {
...
async fn test_list_relationships_with_filters() {
    ...
    let result = vault
        .relationships()
        .list()
        .resource("document:filter1")
        .await;

    match result {
        Ok(response) => {
            println!(
                "Filtered relationships: {} found",
    ...
    match result {
        Ok(response) => {
            println!(
                "Relationships for user:alice: {} found",
                response.relationships.len()
            );
        }
        Err(e) => {
    ...
    match result {
        Ok(resources) => {
            ...
            for resource in &resources {
                assert!(
                    resource.starts_with("document:"),
                    "Should only return document resources"
                );
            }
        }
        Err(e) => {
        ...
        Ok(subjects) => {
            ...
            for subject in &subjects {
                assert!(
                    ...
                    "Should only return user subjects"
                );
            }
        }
        Err(e) => {
...
async fn test_client_health_check() {
    ...
    assert!(
        check_result.is_ok(),
        "Client should be able to perform operations"
    );
...
async fn test_batch_check_with_context() {
    ...
    let checks = vec![
        ("user:alice", "view", "document:ctx1"),
        ("user:bob", "edit", "document:ctx2"),
    ];
...
async fn test_require_permission() {
    ...
    let result = vault
        .check("user:alice", "view", "document:nonexistent")
        .require()
        .await;
