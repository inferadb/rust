...
async fn test_timeout_configuration() {
    ...
    match client {
        Ok(client) => {
            ...
            let result = vault
                .check("user:timeout-test", "view", "document:test")
                .await;
...
async fn test_retry_configuration() {
    ...
    match client {
        Ok(client) => {
            ...
            assert!(
                result.is_ok(),
                "Client with custom retry config should work"
            );
        }
        Err(e) => {
...
async fn test_batch_transport_operations() {
    ...
    let check_resources: Vec<String> = (0..20)
        .map(|i| format!("document:batch-transport{}", i))
        .collect();

    let checks: Vec<(&str, &str, &str)> = (0..20)
        .map(|i| {
            (
                check_subjects[i].as_str(),
                check_permissions[i].as_str(),
                check_resources[i].as_str(),
            )
        })
        .collect();
